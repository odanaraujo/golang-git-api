...
func (ur *userRepository) FindUserByEmailAndPassword(email string, password string) (model.UserDomainInterface, *exception.Exception) {

	logger.Info("init LoginUserService repository", zap.String("Journey", "LoginUserService"))
	...
	if err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			errMessage := "user or password invalid"
			return nil, exception.ForbiddenException(errMessage)
		}

		errMessage := "error trying to find user by email and passworld"
